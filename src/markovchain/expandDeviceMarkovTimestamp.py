...
def compareChosenDevicesByAlarmPriority(cursor):
    ...
    fw = File_writer("validation", config.FILE_SUFFIX, config.EXTRA, "MC")
    fw.create_txt("../../res/")
    ...
    for ts in stopList:
        ...
        if config.WINDOW == "after":
            ...
            events = cursor.fetchall()
        for e in events:
            devices.append(e[4] + "--" + e[5])
        if devices != []:
            seq.append(devices)
            seqPriority.append((devices, 'L0'))
...
def create_sequences_txt():
    cnx = mysql.connector.connect(host='127.0.0.1', user='root', password='password', database='cern')
    cursor = cnx.cursor()
    seqPriority = compareChosenDevicesByAlarmPriority(cursor)
    cursor.close()
    return seqPriority


if __name__ == "__main__":
    create_sequences_txt()